(function () {
    ...
    var injectParams = ['$scope', '$location', '$routeParams',
                        '$timeout', 'config', 'patientsService', 'modalService'];
    ...
    var patientEditController = function ($scope, $location, $routeParams,
        ...
        vm.saveCustomer = function () {
            if ($scope.editForm.$valid) {
                if (!vm.customer.id) {
                    patientsService.insertCustomer(vm.customer).then(processSuccess, processError);
                }
                else {
                    patientsService.updateCustomer(vm.customer).then(processSuccess, processError);
                }
            }
        };
        ...
        function getStates() {
          return patientsService.getStates().then(function (states) {
            vm.states = states;
          }, processError);
        }
        ...
        vm.deleteCustomer = function () {
            ...
            modalService.showModal({}, modalOptions).then(function (result) {
                if (result === 'ok') {
                    patientsService.deleteCustomer(vm.customer.id).then(function () {
        ...
        function init() {
            ...
            getStates().then(function () {
                if (customerId > 0) {
                    patientsService.getCustomer(customerId).then(function (customer) {
                        vm.customer = customer;
                    }, processError);
                } else {
                    patientsService.newCustomer().then(function (customer) {
                        vm.customer = customer;
                    });
                }
            });
